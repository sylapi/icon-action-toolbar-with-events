<template>
    ...
    <div class="icon-action-toolbar">
        ...
        <component v-if="actionModalVisible" :show="actionModalVisible" class="text-left"
            ...
            @close="closeConfirmationModalOverride" />
...
const runAction = () => {
    Nova.$emit('action-modal-executed', {
        selectedAction: selectedAction.value,
        actionUriKey: selectedAction.value.uriKey
    })

    window.dispatchEvent(
        new CustomEvent("nova-action-modal-executed", {
            bubbles: true,
            detail: {
                selectedAction: selectedAction.value,
                actionUriKey: selectedAction.value.uriKey
    ...
    executeAction(() => emitter('actionExecuted'))
}
const parentType = instance.parent.vnode.type.__file

const closeConfirmationModalOverride = () => {
    Nova.$emit('action-modal-closed', {
        selectedAction: selectedAction,
        actionUriKey: selectedAction.value.uriKey
    })

    window.dispatchEvent(
        new CustomEvent("nova-action-modal-closed", {
            bubbles: true,
            detail: {
                selectedAction: selectedAction.value,
                actionUriKey: selectedAction.value.uriKey
...
const onClick = event => {
    ...
    Nova.$emit('action-modal-opened', {
        selectedAction: action,
        actionUriKey: event
    })

    window.dispatchEvent(
        new CustomEvent("nova-action-modal-opened", {
            bubbles: true,
            detail: {
                selectedAction: action,
                actionUriKey: event
...
const availableActions = computed(() => {
    ...
    if (resource) {
        ...
        if (resource.authorizedToReplicate) {
            ...
            actions.push({
                ...
                onClick: () => {
                    ...
                    const url = instance.ctx.$url(
                        `/resources/${props.resourceName}/${resource.id.value}/replicate`, {
        ...
        if (resource.authorizedToDelete && !resource.softDeleted && Nova.$router.page.component !==
            'Nova.Index') {
